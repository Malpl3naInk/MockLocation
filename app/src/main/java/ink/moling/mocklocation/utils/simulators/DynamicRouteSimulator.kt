...
class DynamicRouteSimulator(
    ...
    override fun next(deltaTimeMs: Long): SimulatedLocation {
        // 如果只有一个点或路线已完成，保持静止
        if (route.points.size <= 1 || routeCompleted) {
            return SimulatedLocation(
                lat = currentLat,
                lng = currentLng,
                alt = currentAlt,
                bearing = currentBearing,
                speed = if (routeCompleted) 0.0 else currentSpeed
            )
        }
        ...
        Logger.d("DynamicRouteSimulator", 
            "Point: $currentPointIndex->$nextPointIndex, Progress: %.2f%%, Bearing: %.1f°, Speed: %.1f m/s, Completed: $routeCompleted"
    ...
    private fun moveAlongRoute(distanceM: Double) {
        ...
        while (remainingDistance > 0.001 && currentPointIndex < route.points.size) {
